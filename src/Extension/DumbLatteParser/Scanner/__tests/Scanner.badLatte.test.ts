...
test('Scanner: Bad Latte: Unclosed Latte tag', () => {
	const scanner = new Scanner(readTestDataFile('badLatte.latteTag.unclosed.latte'), true)
	expect(() => scanner.scan()).toThrow('Offset 28: Missing end of latte_tag region')
})

test('Scanner: Bad Latte: Unclosed HTML tag', () => {
	const scanner = new Scanner(readTestDataFile('badLatte.htmlTag.unclosed.latte'), true)
